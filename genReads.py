...
from inputChecking import requiredField, checkFileOpen, isInRange
...
if PAIRED_END and PAIRED_END_ARTIFICIAL:
    ...
    if FRAGMENT_STD == 0:
        ...
        potential_vals = list(range(FRAGMENT_SIZE - 6 * FRAGMENT_STD, FRAGMENT_SIZE + 6 * FRAGMENT_STD + 1))
        FRAGLEN_VALS = []
        for i in range(len(potential_vals)):
            if potential_vals[i] > READLEN:
                FRAGLEN_VALS.append(potential_vals[i])
...
def main():
    ...
    if INPUT_BED != None:
        ...
        for k in refList:
            if k not in inputRegions:
                nInRefOnly += 1
        for k in list(inputRegions.keys()):
            if not k in refList:
                nInBedOnly += 1
                del inputRegions[k]
        if nInRefOnly > 0:
    ...
    if NJOBS > 1:
        ...
        corrected_nJobs = min([NJOBS, sum([len(n) for n in list(parallelRegionList.values())])])
    else:
        corrected_nJobs = 1
    ...
    if CANCER:
        OFW = OutputFileWriter(OUT_PREFIX + '_normal', paired=PAIRED_END, BAM_header=bamHeader, VCF_header=vcfHeader,
                               gzipped=GZIPPED_OUT, noFASTQ=NO_FASTQ)
